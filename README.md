...
[![Version](https://img.shields.io/badge/version-0.6.13-b91c1c?style=flat-square)](./manifest.json)
...
```text
output/x-bookmark-organizer-0.6.13.crx
output/x-bookmark-organizer-0.6.13/
```
...
3. 打开右上角「开发者模式」。
4. 点击「加载已解压的扩展程序」。
5. 选择 `E:\BaiduNetdiskDownload\X_bookmark\output\x-bookmark-organizer-0.6.13`。
6. 安装完成后，工具栏会出现插件图标。
...
- 开发者：[ksk2023](https://github.com/ksk2023)。我负责这个插件的开发与维护，也负责把每一个“这里还能不能更顺手一点”的想法继续往前推。
- Token 供应商：[cnYui](https://github.com/cnYui)。感谢提供 Token 支持，让 AI 分类功能不只是页面上的按钮，而是真的能在使用时跑起来。

也感谢每一次真实使用中的截图、反馈和“继续继续”。它们让很多“差不多能用”的地方，慢慢变成“确实更好用一点”。
